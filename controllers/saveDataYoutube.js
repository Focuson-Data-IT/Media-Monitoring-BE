...
const saveDataPostByKeywords = async (post) => {
    const sql = `
        ...
        thumbnail_url, title, caption, comments,
        playCount, collectCount, likes, shareCount)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON DUPLICATE KEY UPDATE
            client_account = VALUES(client_account),
            kategori = VALUES(kategori),
    ...
    connection.query(sql, [
        ...
        post.thumbnail_url, post.title, post.caption, post.comments,
        post.playCount, post.collectCount, post.likes, post.shareCount
    ], (err, result) => {
        if (err) {
            console.error(`❌ Error saving post ${post.unique_id_post} to database:`, err.message);
        } else {
